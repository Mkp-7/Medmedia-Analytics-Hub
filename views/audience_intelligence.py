...
ENGAGEMENT = {
    "Oncology":          {"hcp": 48000, "or": 34.2},
    "Cardiology":        {"hcp": 32000, "or": 31.8},
    "Neurology":         {"hcp": 24000, "or": 29.4},
    "Immunology":        {"hcp": 19000, "or": 33.1},
    "Rare Disease":      {"hcp":  9000, "or": 38.7},
    "Endocrinology":     {"hcp": 15000, "or": 30.5},
    "Infectious Disease":{"hcp": 12000, "or": 27.9},
    "Gastroenterology":  {"hcp": 10000, "or": 28.4},
    "Pulmonology":       {"hcp": 11000, "or": 26.8},
    "Hematology":        {"hcp":  8000, "or": 35.2},
}
...
def render():
    page_header("Audience Intelligence", "Specialty reach, geographic concentration, and sponsor targeting intelligence", "👥")
    ...
    with left:
        ...
        fig = go.Figure(go.Bar(
            x=[s[1] for s in sorted_specs],
            y=[s[0] for s in sorted_specs],
            orientation="h",
            marker=dict(color=COLORS[:len(sorted_specs)][::-1]),
            text=[f"{v:,}" for v in [s[1] for s in sorted_specs]],
            textposition="outside",
        ))
        fig.update_layout(
            height=360, margin=dict(l=0,r=50,t=10,b=10),
    ...
    with right:
        ...
        fig2 = go.Figure(go.Bar(
            x=list(US_STATES.values()), y=list(US_STATES.keys()),
            orientation="h",
            marker=dict(color=list(US_STATES.values()), colorscale=[[0,"#1a3a5c"],[1,"#378ADD"]], showscale=False),
            text=list(US_STATES.values()), textposition="outside",
        ))
        fig2.update_layout(
            height=360, margin=dict(l=0,r=50,t=10,b=10),
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    ...
    st.markdown("---")
    section_header("📧 HCP engagement metrics by specialty")
    hdr = st.columns(4)
        ...
        with sp_cols[i % 3]:
            st.markdown(f"""<div class="sponsor-card">
                ...
                <div style="margin-top:6px">
                    <span class="badge badge-green">Ad target</span>
                    <span class="badge badge-blue">Content partner</span>
                </div>
