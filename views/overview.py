...
from utils.ui import page_header, trial_card, section_header
...
def render():
    ...
    with left:
        ...
        fig.update_layout(
            height=280, margin=dict(l=0,r=50,t=10,b=10),
        ...
        fig2 = go.Figure(go.Bar(
            x=[t[0].split()[0].capitalize() for t in sorted_t],
            y=[t[1] for t in sorted_t],
            marker=dict(color=[t[1] for t in sorted_t], colorscale=[[0,"#1a3a5c"],[1,"#378ADD"]]),
            text=[f"{t[1]:,}" for t in sorted_t],
            textposition="outside",
        ))
        fig2.update_layout(
